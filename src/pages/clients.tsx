      ...
      {loading ? (
        ...
        <>
          {clientUsers.length > 0 ? (
            <UserTable users={clientUsers} createAction={handleOpenModal} />
          ) : (
            <NoClientsPage createNewClient={handleOpenModal} />
          )}
        </>
      )}
      <NewUserModal open={isModalOpen} onClose={handleCloseModal} role="client" />
    </PageContainer>
  );
}
...
function NoClientsPage({ createNewClient }: { createNewClient: () => void }) {
  return (
    <Box>
      ...
      <Button variant="contained" color="primary" onClick={createNewClient} sx={{ mt: 2 }}>
        Create New Client
      </Button>
    </Box>
  );
