...
const NewUserModal: React.FC<NewUserModalProps> = ({ open, onClose, role }) => {
	...
	const handleSubmit = async (event: React.FormEvent) => {
		...
		try {
			...
			notifications.show(`${role.charAt(0).toUpperCase() + role.slice(1)} member created successfully`, {
				severity: "success",
	...
	return (
		<ReusableModal open={open} onClose={onClose} title={`Create New ${role.charAt(0).toUpperCase() + role.slice(1)}`}>
